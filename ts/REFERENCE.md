# MeteoprogWeather TypeScript SDK Reference

Complete API reference for the MeteoprogWeather TypeScript SDK.
...
```ts
new MeteoprogWeatherSDK(options?: object)
```
...
#### `MeteoprogWeatherSDK.test(testopts?, sdkopts?)`
...
**Returns:** `MeteoprogWeatherSDK` instance in test mode.
...
Alias for `MeteoprogWeatherSDK.test()`.
...
Return the parent `MeteoprogWeatherSDK` instance.
...
```ts
const results = await client.Historical().list({ end_date: "example", start_date: "example" })
```
