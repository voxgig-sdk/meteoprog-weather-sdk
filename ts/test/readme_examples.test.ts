...
//     (\`new MeteoprogWeatherSDK(...)\` / \`MeteoprogWeatherSDK.test(...)\` become
//     \`MeteoprogWeatherSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'MeteoprogWeatherSDK'
...
const TEST_SEED = {"entity":{"current":{"test01":{"id":"test01"}},"historical":{"test01":{"id":"test01"}},"weather_forecast":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new MeteoprogWeatherSDK(...)\` and any
// \`MeteoprogWeatherSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+MeteoprogWeatherSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/MeteoprogWeatherSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
