...
describe('CurrentDirect', async () => {
  ...
  test('direct-load-current', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-current', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['METEOPROG_WEATHER_TEST_CURRENT_ENTID']
