...
describe('CurrentEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'current.' + op, live)) return
    }
    ...
    const current_ref01_data_dt0 = await current_ref01_ent.load(current_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['METEOPROG_WEATHER_TEST_CURRENT_ENTID']
