# MeteoprogWeather TypeScript SDK



The TypeScript SDK for the MeteoprogWeather API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.Current()` — each with a small set of operations (`list`, `load`)
...
```ts
const client = new MeteoprogWeatherSDK({ apikey: '...' })
...
```ts
new MeteoprogWeatherSDK(options?: {
  apikey?: string
  base?: string
  prefix?: string
  suffix?: string
...
| `tester(testopts?, sdkopts?)` | `MeteoprogWeatherSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `MeteoprogWeatherSDK.test(testopts?, sdkopts?)` | `MeteoprogWeatherSDK` | Create a test-mode client. |
...
| `client` | `client(): MeteoprogWeatherSDK` | Return the parent SDK client. |
...
```ts
const historicals = await client.Historical().list({ end_date: "example", start_date: "example" })
```
...
meteoprog-weather/
├── src/
│   ├── MeteoprogWeatherSDK.ts        # Main SDK class
