...
module MeteoprogWeatherUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = MeteoprogWeatherResponse.new({})
    ...
    if fetch_err
      ...
      response = MeteoprogWeatherResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = MeteoprogWeatherResponse.new(fetched)
    else
