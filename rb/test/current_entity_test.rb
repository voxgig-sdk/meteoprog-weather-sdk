...
class CurrentEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = current_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "current." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set METEOPROG_WEATHER_TEST_CURRENT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    current_ref01_data = nil
    if current_ref01_data_raw.length > 0
...
def current_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "current", "CurrentTestData.json")
  ...
  entid_env_raw = ENV["METEOPROG_WEATHER_TEST_CURRENT_ENTID"]
