...
class HistoricalEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = MeteoprogWeatherSDK.test(seed, nil)
    seen = base.Historical(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Historical(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set METEOPROG_WEATHER_TEST_HISTORICAL_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    historical_ref01_list_result = historical_ref01_ent.list(historical_ref01_match, nil)
    assert historical_ref01_list_result.is_a?(Array)
...
def historical_basic_setup(extra)
  ...
  entid_env_raw = ENV["METEOPROG_WEATHER_TEST_HISTORICAL_ENTID"]
