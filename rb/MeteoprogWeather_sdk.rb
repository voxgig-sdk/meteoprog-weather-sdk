...
require_relative 'MeteoprogWeather_types'
...
class MeteoprogWeatherSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = MeteoprogWeatherHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = MeteoprogWeatherHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, MeteoprogWeatherFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    MeteoprogWeatherUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = MeteoprogWeatherHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = MeteoprogWeatherHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = MeteoprogWeatherHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue MeteoprogWeatherError => err
    ...
    if fetched.is_a?(Hash)
      status = MeteoprogWeatherHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = MeteoprogWeatherError.new(
        "graphql_error", "MeteoprogWeatherSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = MeteoprogWeatherSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
