...
class CurrentEntity
  ...
  def make
    opts = @_entopts.dup
    CurrentEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = MeteoprogWeatherHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Current, Hash] the current Current data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = MeteoprogWeatherHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Current fields)
  def match_get
  ...
  #
  # @param reqmatch [CurrentLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Current.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Current, Hash] the loaded Current; raises MeteoprogWeatherError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = MeteoprogWeatherHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
