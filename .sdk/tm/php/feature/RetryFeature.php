...
// MeteoprogWeather SDK retry feature
...
class MeteoprogWeatherRetryFeature extends MeteoprogWeatherBaseFeature
{
    ...
    public function init(MeteoprogWeatherContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (MeteoprogWeatherContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(MeteoprogWeatherContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(MeteoprogWeatherContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
