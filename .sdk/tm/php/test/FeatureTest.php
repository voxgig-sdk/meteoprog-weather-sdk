...
// MeteoprogWeather SDK feature test
//
...
class FtCtrl extends MeteoprogWeatherControl
{
...
class FtClient
{
    ...
    private MeteoprogWeatherUtility $utility;

    public function __construct(MeteoprogWeatherUtility $utility, array $options)
    {
    ...
    public function get_utility(): MeteoprogWeatherUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MeteoprogWeatherContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public MeteoprogWeatherContext $rootctx;
    ...
    public function __construct(
        ...
        $utility = new MeteoprogWeatherUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (MeteoprogWeatherContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MeteoprogWeatherConfig::make_feature($name);
    ...
    private static function buildUrl(MeteoprogWeatherSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MeteoprogWeatherError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MeteoprogWeatherResponse($fetched);
        }
    ...
    private function fail(MeteoprogWeatherContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MeteoprogWeatherContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
