...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        MeteoprogWeatherSDK $client,
        MeteoprogWeatherUtility $utility
    ): MeteoprogWeatherContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new MeteoprogWeatherContext($ctxmap, null);
    ...
    private static function fixctx(MeteoprogWeatherContext $ctx, MeteoprogWeatherSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?MeteoprogWeatherError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new MeteoprogWeatherError($code, $msg);
    }
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new MeteoprogWeatherResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new MeteoprogWeatherBaseFeature();
    ...
    {
        $hook_client = MeteoprogWeatherSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends MeteoprogWeatherBaseFeature {
            public $hook_fn = null;
            public function TestHook(MeteoprogWeatherContext $ctx): void
            {
    ...
    {
        $init_client = MeteoprogWeatherSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends MeteoprogWeatherBaseFeature {
            public $init_fn = null;
    ...
    {
        $calls = [];
        $live_client = new MeteoprogWeatherSDK([
    ...
    {
        $blocked_client = new MeteoprogWeatherSDK([
            'base' => 'http://localhost:8080',
            'system' => [
    ...
    public function test_make_fetch_def_basic(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new MeteoprogWeatherSpec([
            'base' => 'http://localhost:8080',
            'prefix' => '/api',
            'path' => 'items/{id}',
            'suffix' => '',
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new MeteoprogWeatherSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = MeteoprogWeatherSDK::test(null, $setup_opts);
    ...
    public function test_operator_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'operator', 'basic'), function (array $entry) {
            $in = $entry['in'] ?? [];
            $op = new MeteoprogWeatherOperation($in);
            return [[
                'entity' => $op->entity,
                'name' => $op->name,
                'input' => $op->input,
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = MeteoprogWeatherSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof MeteoprogWeatherError) ? $result->err->msg : (string)$result->err;
