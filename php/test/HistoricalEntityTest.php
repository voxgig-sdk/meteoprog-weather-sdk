...
class HistoricalEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Historical(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Historical(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set METEOPROG_WEATHER_TEST_HISTORICAL_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $historical_ref01_list_result = $historical_ref01_ent->list($historical_ref01_match, null);
        $this->assertIsArray($historical_ref01_list_result);
...
function historical_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("METEOPROG_WEATHER_TEST_HISTORICAL_ENTID");
