...
// MeteoprogWeather SDK Current entity
...
class CurrentEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new CurrentEntity($this->_client, $opts);
    }

    /**
     * @param Current|array $args Current data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = MeteoprogWeatherHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Current|array The current Current data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Current fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = MeteoprogWeatherHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Current fields).
     */
    ...
    /**
     * Load a single Current.
     *
     * @param CurrentLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed CurrentLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Current|array The loaded Current as an assoc-array at the
     *   SDK boundary; throws MeteoprogWeatherError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = MeteoprogWeatherHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
