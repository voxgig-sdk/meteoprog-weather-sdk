...
require("meteoprog-weather_types")
...
local MeteoprogWeatherSDK = {}
MeteoprogWeatherSDK.__index = MeteoprogWeatherSDK
...
MeteoprogWeatherSDK._make_feature = _make_feature


function MeteoprogWeatherSDK.new(options)
  local self = setmetatable({}, MeteoprogWeatherSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function MeteoprogWeatherSDK:options_map()
...
function MeteoprogWeatherSDK:get_utility()
  return Utility.copy(self._utility)
end


function MeteoprogWeatherSDK:get_root_ctx()
  return self._rootctx
end


function MeteoprogWeatherSDK:prepare(fetchargs)
...
function MeteoprogWeatherSDK:direct(fetchargs)
...
function MeteoprogWeatherSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function MeteoprogWeatherSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "MeteoprogWeatherSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function MeteoprogWeatherSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function MeteoprogWeatherSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "MeteoprogWeatherSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function MeteoprogWeatherSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = MeteoprogWeatherSDK.new(sdkopts)
  sdk.mode = "test"
