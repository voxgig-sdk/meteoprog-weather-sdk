...
describe("HistoricalEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Historical(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Historical(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set METEOPROGWEATHER_TEST_HISTORICAL_ENTID JSON to run live")
      return
    end
    ...
    local historical_ref01_match = {}

    local historical_ref01_list_result, err = historical_ref01_ent:list(historical_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(historical_ref01_list_result)
...
function historical_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("METEOPROGWEATHER_TEST_HISTORICAL_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["METEOPROGWEATHER_TEST_HISTORICAL_ENTID"])
