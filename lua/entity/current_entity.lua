...
local CurrentEntity = {}
CurrentEntity.__index = CurrentEntity


function CurrentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CurrentEntity)
  self._name = "current"
  self._client = client
...
function CurrentEntity:get_name()
  return self._name
end


function CurrentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CurrentEntity.new(self._client, opts)
end
...
function CurrentEntity:mark_deleted()
  self._deleted = true
end


function CurrentEntity:deleted()
  return true == self._deleted
end


function CurrentEntity:data_set(args)
  if args ~= nil then
...
function CurrentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CurrentEntity:match_set(args)
  if args ~= nil then
...
function CurrentEntity:match_get()
...
function CurrentEntity:stream(action, args, callopts)
...
---@param reqmatch CurrentLoadMatch
---@param ctrl? table
---@return Current
---@return string? err
function CurrentEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function CurrentEntity:_run_op(ctx, post_done)
  local utility = self._utility
