...
The Lua SDK for the MeteoprogWeather API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Current(nil):load({ id = "example_id" }, nil)
...
local result, err = client:MeteoprogWeather(nil):load(
  { id = "test01" }, nil
)
