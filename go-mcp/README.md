# meteoprog-weather-mcp

[MCP](https://modelcontextprotocol.io) server exposing the MeteoprogWeather SDK as
two agent tools — `meteoprog-weather_list` and `meteoprog-weather_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/meteoprog-weather-mcp)
make build

# 2. Provide credentials via the environment
export METEOPROG_WEATHER_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user meteoprog-weather \
  -- /absolute/path/to/meteoprog-weather-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./meteoprog-weather-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// meteoprog-weather_list: first page of records
{ "entity": "historical" }
{ "entity": "historical", "query": { } }

// meteoprog-weather_load: one record by id
{ "entity": "current", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/meteoprog-weather-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user meteoprog-weather \
     -- "$PWD"/dist/*/meteoprog-weather-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `meteoprog-weather_list` and `meteoprog-weather_load` tools now appear
   in new sessions. Ask the agent to *"list historical using meteoprog-weather"*
   and it calls `meteoprog-weather_list` with `{"entity":"historical"}`.
...
```sh
export METEOPROG_WEATHER_APIKEY=sk_live_xxx            # API key
export METEOPROG_WEATHER_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `meteoprog-weather_list` tool
...
### Call the `meteoprog-weather_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `meteoprog-weather_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `meteoprog-weather_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `METEOPROG_WEATHER_APIKEY` | API key sent with every request. |
| `METEOPROG_WEATHER_BASE` | Optional override of the API base URL. |

### Entities

The 3 entities valid as the `entity` argument:

current | historical | weather_forecast

### Smoke test via HTTP (raw JSON-RPC)

```sh
./meteoprog-weather-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"meteoprog-weather_load","arguments":{"entity":"current","query":{"id":1}}}}'
```
