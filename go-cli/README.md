# meteoprog-weather-cli

boru-driven command-line client **and** interactive REPL for the MeteoprogWeather
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/meteoprog-weather-cli)
make build

# 2. See usage (words, entities, env vars)
./meteoprog-weather-cli --help

# 3. Provide credentials once, via the environment
export METEOPROG_WEATHER_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./meteoprog-weather-cli load 1 current            # {id:1} shorthand
./meteoprog-weather-cli load '{id:1}' current       # explicit match map

# 5. Override the API base URL for a single call
METEOPROG_WEATHER_BASE=https://api.example.com ./meteoprog-weather-cli load 1 current

# 6. No arguments -> interactive REPL
./meteoprog-weather-cli
meteoprog-weather> load 1 current
meteoprog-weather> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/meteoprog-weather-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/meteoprog-weather-cli load 1 current
   ```
...
```sh
./meteoprog-weather-cli load 1 current          # scalar shorthand for {id:1}
./meteoprog-weather-cli load '{id:1}' current     # explicit match map
```
...
./meteoprog-weather-cli load 1 current
```
...
Run with no arguments to open a REPL (prompt `meteoprog-weather>`). Each line is
evaluated as its own boru expression:

```text
$ ./meteoprog-weather-cli
meteoprog-weather> load 1 current
meteoprog-weather> /help
meteoprog-weather> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `current`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/meteoprog-weather-cli`. |
...
(`./meteoprog-weather-cli <expr>`) and interactively (the REPL), and expressions compose the
