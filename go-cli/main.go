// Command MeteoprogWeather-cli is a CLI + REPL for the MeteoprogWeather SDK,
...
const prompt = "meteoprog-weather"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "current historical weather_forecast"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      METEOPROG_WEATHER_APIKEY (api key), METEOPROG_WEATHER_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: METEOPROG_WEATHER_APIKEY carries the API key and
	// METEOPROG_WEATHER_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("METEOPROG_WEATHER_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("METEOPROG_WEATHER_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewMeteoprogWeatherSDK(opts)
