...
The Python SDK for the MeteoprogWeather API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Current(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.MeteoprogWeather(None).load(
    {"id": "test01"}, None
)
