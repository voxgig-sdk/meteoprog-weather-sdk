...
class TestWeatherForecastDirect:
    ...
    def test_should_direct_list_weather_forecast(self):
        setup = _weather_forecast_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-weather_forecast", "live" if setup["live"] else "unit")
        if _skip:
