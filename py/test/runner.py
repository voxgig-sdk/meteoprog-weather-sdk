# MeteoprogWeather SDK test runner
...
class MeteoprogWeatherTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MeteoprogWeatherTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MeteoprogWeatherTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MeteoprogWeatherTestRunner.getenv("METEOPROG_WEATHER_TEST_LIVE")
        override = MeteoprogWeatherTestRunner.getenv("METEOPROG_WEATHER_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MeteoprogWeatherTestRunner.getenv(key)
        ...
        explain = MeteoprogWeatherTestRunner.getenv("METEOPROG_WEATHER_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if MeteoprogWeatherTestRunner._test_control is not None:
            return MeteoprogWeatherTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MeteoprogWeatherTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MeteoprogWeatherTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MeteoprogWeatherTestRunner.load_test_control()
...
def load_env_local():
    MeteoprogWeatherTestRunner.load_env_local()


def env_override(m):
    return MeteoprogWeatherTestRunner.env_override(m)


def entity_data(v):
    return MeteoprogWeatherTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return MeteoprogWeatherTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MeteoprogWeatherTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MeteoprogWeatherTestRunner.load_test_control()


def live_delay_ms():
    return MeteoprogWeatherTestRunner.live_delay_ms()
