...
class TestHistoricalDirect:
    ...
    def test_should_direct_list_historical(self):
        setup = _historical_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-historical", "live" if setup["live"] else "unit")
        if _skip:
