...
class TestCurrentEntity:

    def test_should_create_instance(self):
        testsdk = MeteoprogWeatherSDK.test(None, None)
        ent = testsdk.Current(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _current_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "current." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set METEOPROG_WEATHER_TEST_CURRENT_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        current_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.current")))
        current_ref01_data = None
        if len(current_ref01_data_raw) > 0:
...
def _current_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/current/CurrentTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("METEOPROG_WEATHER_TEST_CURRENT_ENTID"))
    ...
    if env.get("METEOPROG_WEATHER_TEST_LIVE") == "TRUE":
        ...
        client = MeteoprogWeatherSDK(helpers.to_map(merged_opts))

    _live = env.get("METEOPROG_WEATHER_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("METEOPROG_WEATHER_TEST_EXPLAIN") == "TRUE",
        "live": _live,
