...
class TestCurrentDirect:

    def test_should_direct_load_current(self):
        setup = _current_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-current", "live" if setup["live"] else "unit")
        if _skip:
...
def _current_direct_setup(mockres):
