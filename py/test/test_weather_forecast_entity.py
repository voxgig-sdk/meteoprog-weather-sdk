...
class TestWeatherForecastEntity:
    ...
    def test_should_stream(self):
        ...
        base = MeteoprogWeatherSDK.test(seed, None)
        seen = list(base.WeatherForecast(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = MeteoprogWeatherSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.WeatherForecast(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set METEOPROG_WEATHER_TEST_WEATHER_FORECAST_ENTID JSON to run live")
        client = setup["client"]
...
def _weather_forecast_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("METEOPROG_WEATHER_TEST_WEATHER_FORECAST_ENTID"))
